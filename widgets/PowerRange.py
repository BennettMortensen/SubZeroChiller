...
class PowerRange:
    def __init__(self, parent, low, high, onRangeChange):
        ...
        self.lowAddBtn = IconButton(lowFrame, os.path.abspath('chiller/assets/add.png'), command=lambda: self.increaseRange('low'))
        ...
        lowLabel.pack()
        self.lowMinusBtn = IconButton(lowFrame, os.path.abspath('chiller/assets/minimize.png'), command=lambda: self.decreaseRange('low'))
        ...
        self.highAddBtn = IconButton(highFrame, os.path.abspath('chiller/assets/add.png'), command=lambda: self.increaseRange('high'))
        ...
        highLabel.pack()
        self.highMinusBtn = IconButton(highFrame, os.path.abspath('chiller/assets/minimize.png'), command=lambda: self.decreaseRange('high'))
    ...
    def decreaseRange(self, range):
        if range == 'low':
            ...
            if self.highRange <=2 or self.highRange - 1 <= self.lowRange:
                return
            self.highRange -= 1
            self.highRangeLabel.config(text=str(self.highRange))
            self.onRangeChange(self.lowRange, self.highRange)
